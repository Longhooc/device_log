...
import smartphApi from '../api/smartphApi';

export const useSmartPhLinks = () => {
    ...
    const loadLinks = useCallback(async () => {
        ...
        try {
            const data = await smartphApi.getLinks(filters);
            setLinks(data);
        } catch (err) {
            setError(err.message);
            console.error('Error loading SmartPH links:', err);
        } finally {
            setLoading(false);
        }
    ...
    const createLink = useCallback(async (linkData) => {
        try {
            setLoading(true);
            const result = await smartphApi.createLink(linkData);
    ...
    const updateLink = useCallback(async (id, linkData) => {
        try {
            setLoading(true);
            const result = await smartphApi.updateLink(id, linkData);
            await loadLinksRef.current(); // Reload to get updated list
            return result;
        } catch (err) {
            setError(err.message);
    ...
    const deleteLink = useCallback(async (id, authCode) => {
        try {
            setLoading(true);
            const result = await smartphApi.deleteLink(id, authCode);
            await loadLinksRef.current(); // Reload to get updated list
            return result;
        } catch (err) {
            setError(err.message);
    ...
    const toggleFavorite = useCallback(async (id, isFavorite) => {
        try {
            const result = await smartphApi.toggleFavorite(id, isFavorite);
    ...
    const incrementAccess = useCallback(async (id) => {
        try {
            const result = await smartphApi.incrementAccess(id);
    ...
    const getStats = useCallback(async () => {
        try {
            return await smartphApi.getStats();
        } catch (err) {
            setError(err.message);
            throw err;
        }
