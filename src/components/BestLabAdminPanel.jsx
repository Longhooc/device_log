...
function BestLabAdminPanel({ onClose }) {
    ...
    const loadLinkPermissionUsers = async (linkId) => {
        try {
            const response = await fetch(`${process.env.REACT_APP_API_URL || 'https://quantrac.online:9443'}/api/bestlab/links/${linkId}/permissions/users`, {
                headers: {
    ...
    const handleAddUserToLink = async (linkId, userId) => {
        try {
            setIsLoading(true);
            await fetch(`${process.env.REACT_APP_API_URL || 'https://quantrac.online:9443'}/api/bestlab/links/${linkId}/permissions/users`, {
                method: 'POST',
                headers: {
                    'Authorization': `Bearer ${localStorage.getItem('authToken')}`,
                    'Content-Type': 'application/json'
    ...
    const handleRemoveUserFromLink = async (linkId, userId) => {
        if (window.confirm('Bạn có chắc chắn muốn xóa quyền truy cập BestLab link này?')) {
            try {
                setIsLoading(true);
                await fetch(`${process.env.REACT_APP_API_URL || 'https://quantrac.online:9443'}/api/bestlab/links/${linkId}/permissions/users/${userId}`, {
                    method: 'DELETE',
                    headers: {
                        'Authorization': `Bearer ${localStorage.getItem('authToken')}`,
                        'Content-Type': 'application/json'
