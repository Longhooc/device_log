...
function SmartPhAdminPanel({ onClose }) {
    ...
    const loadLinkPermissionUsers = async (linkId) => {
        try {
            ...
            console.error('Error loading SmartPH link permission users:', error);
    ...
    // Function để thêm user vào SmartPH link permissions
    const addUserToSmartPhLinkPermissions = async (linkId, userId) => {
        try {
            const token = localStorage.getItem('authToken');
            const response = await fetch(`/api/smartph/links/${linkId}/permissions/users`, {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
            ...
            console.log(`✅ Added user ${userId} to SmartPH link ${linkId}:`, result);
            return result;
        } catch (error) {
            console.error(`❌ Error adding user ${userId} to SmartPH link ${linkId}:`, error);
            throw error;
        }
    };
    ...
    const handleAddUser = async () => {
        ...
        try {
            ...
            // 2. Lấy tất cả SmartPH links của phòng ban
            const allLinks = smartphLinks.links; // smartphLinks.links là array từ useSmartPhLinks hook
            const departmentLinks = allLinks.filter(link => link.department === newUser.department);
            console.log(`📋 Found ${departmentLinks.length} SmartPH links for department: ${newUser.department}`);
            
            // 3. Cấp quyền truy cập tất cả SmartPH links của phòng ban cho user mới
            if (departmentLinks.length > 0) {
                const permissionPromises = departmentLinks.map(link => 
                    addUserToSmartPhLinkPermissions(link.id, createdUser.id).catch(error => {
                        console.warn(`⚠️ Failed to grant access to SmartPH link ${link.id}:`, error);
                        return null; // Continue with other links even if one fails
                    })
                );
                
                await Promise.all(permissionPromises);
                console.log(`✅ Granted access to ${departmentLinks.length} SmartPH links for user ${createdUser.username}`);
            }
            ...
            alert(`Thêm người dùng thành công! Đã cấp quyền truy cập ${departmentLinks.length} tài liệu SmartPH của phòng ban.`);
        } catch (error) {
    ...
    const handleAddUserToLink = async (linkId, userId) => {
        try {
            ...
            alert('Thêm quyền truy cập SmartPH link thành công!');
        } catch (error) {
            console.error('Error adding user to SmartPH link permissions:', error);
    ...
    const handleRemoveUserFromLink = async (linkId, userId) => {
        if (window.confirm('Bạn có chắc chắn muốn xóa quyền truy cập SmartPH link này?')) {
            try {
                ...
                await loadLinkPermissionUsers(linkId);
                alert('Xóa quyền truy cập SmartPH link thành công!');
            } catch (error) {
                console.error('Error removing user from SmartPH link permissions:', error);
            ...
            <div className="admin-panel">
                ...
                {activeTab === 'permissions' && (
                    <div className="permissions-tab">
                        <div className="tab-header">
                            <h2>Quyền truy cập SmartPH links</h2>
                            <p>Chọn SmartPH link và quản lý quyền truy cập ngay bên cạnh</p>
                        </div>

                        <div className="permissions-layout">
                            {/* Left: SmartPH Link list */}
                            <div className="permissions-left">
                                <div className="link-selection">
                                    <div className="links-grid">
                                        {smartphLinks.links.map(link => (
                                            <div 
                                                key={link.id}
                            ...
                            <div className="permissions-right">
                                <div className="permission-management">
                                    <h3>{selectedLinkId ? 'Quản lý quyền truy cập SmartPH link' : 'Chọn một SmartPH link để quản lý quyền'}</h3>

                                    {selectedLinkId && (
                                        <>
                                            <div className="add-user-section">
                                                <h4>Thêm tài khoản có quyền truy cập SmartPH link:</h4>
                                            ...
                                            <div className="current-permissions">
                                                <h4>Tài khoản hiện có quyền truy cập SmartPH link:</h4>
...
export default SmartPhAdminPanel;